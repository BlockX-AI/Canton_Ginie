"""Load invite codes into database - Railway compatible.

Run this from the backend directory:
    python load_codes.py
"""
...
def main():
    print("🔄 Loading invite codes into database...")
    ...
    try:
        with get_db_session() as session:
            ...
            print(f"\n✅ Successfully loaded invite codes!")
            print(f"   Added: {added}")
            print(f"   Skipped (already exist): {skipped}")
            print(f"   Total: {len(INVITE_CODES)}")
            
            # Show first 5 codes
            print(f"\n📋 First 5 codes:")
            for i, code in enumerate(INVITE_CODES[:5], 1):
                print(f"   {i}. {code}")
            
    except Exception as e:
        print(f"\n❌ Error loading codes: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
