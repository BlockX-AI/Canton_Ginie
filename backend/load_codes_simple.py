"""Simple script to load invite codes - runs on Railway only.

This script should be run in Railway's environment where all dependencies are installed.
"""
...
print("🔄 Loading 50 invite codes...")

with get_db_session() as session:
    added = 0
    for code in CODES:
        if not session.query(InviteCode).filter_by(code=code).first():
    ...
    print(f"✅ Added {added} new codes")
    print(f"📊 Total codes in database: {total}")
    print(f"\n🎉 Ready! Try signup with: {CODES[0]}")
